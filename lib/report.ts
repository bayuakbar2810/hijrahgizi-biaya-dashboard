...
/* Narasi penyebab ' bahasa sederhana, arah penyimpangan dijelaskan dengan kata. */
...
function kpiBox(
  ...
  let vs = 13;
  doc.setFontSize(vs);
  while (doc.getTextWidth(value) > w - 8 && vs > 8) {
    vs -= 0.5;
    doc.setFontSize(vs);
  }
...
function drawEvidence(
  ...
  /* kotak angka utama ' label menyesuaikan jenis anomali */
  const curLabel =
    a.type === "LOW_YIELD"
      ? "Yield sekarang"
  ...
  autoTable(doc, {
    ...
    theme: "grid",
    styles: { fontSize: 8.5, cellPadding: 1.5, textColor: INK },
    headStyles: { fillColor: BRAND, textColor: 255, fontSize: 8.5, fontStyle: "bold" },
    footStyles: { fillColor: BAND, textColor: INK, fontStyle: "bold", fontSize: 8.5 },
...
export function generateReportPdf(
  ...
  if (mode === "lengkap") {
    ...
    autoTable(doc, {
      ...
      theme: "grid",
      styles: { fontSize: 8.5, cellPadding: 1.6, textColor: INK },
...
export function generateBahanDetailPdf(
  ...
  for (const sku of skus) {
    ...
    autoTable(doc, {
      startY: y,
      head: [["Bahan", "Kode", "Berapa kali dipakai (batch)", "Total qty", "Total biaya (Rp)", "Biaya satuan rata2 (Rp)", "Terakhir dipakai", "Stok gudang (kg)", "Letak gudang"]],
      body: sku.rows.map((r) => [
        r.nama,
        r.kode,
        fmtNum(r.nBatch, 0),
